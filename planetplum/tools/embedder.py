...
def bandcamp(url):
    ...
    if video.is_valid():
        try:
            ...
            html = f'''<iframe src="{escaped_src}" style="margin: .5rem auto; text-align: center; display:block; width:100%; max-width: {video['video:width']}px;" height="{video['video:height']}" scrolling="no" frameborder="0" allowfullscreen></iframe>'''
            return html
        except:
            return ""
